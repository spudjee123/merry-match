...
console.log(complaint_id);
  ...
  const getComplaintDetail = async () => {
    try {
      const result = await axios.get(`http://localhost:4001/complaint/${complaint_id}`);
      ...
      <div className="flex flex-col w-screen ml-5">
        <div className="px-4 lg:px-[60px] py-4 bg-white border-b border-[#d6d9e4] flex justify-between items-center">
          <div className="flex items-center gap-4">
            <button onClick={handleClickToCompalint}>
          ...
          <div className="flex items-center gap-4">
            <button className="px-2 py-1 rounded-2xl flex items-center">
              <div className="text-[#c70039] text-base font-bold" onClick={handleClickToCompalint}>
                Cancel Complaint
              </div>
            </button>
