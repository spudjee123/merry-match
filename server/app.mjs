...
app.get("/membership/:name", async (req, res) => {
  ...
  try {
    const memberShipData = await connectionPool.query(
      `SELECT pt.name, pt.packages_name, pt.created_date, p.merry_limit, p.icons, p.detail, p.price
      FROM payment_test pt
      LEFT JOIN packages p on p.packages_name = pt.packages_name 
      WHERE pt.name = $1 AND pt.status = 'success' 
      ORDER BY pt.created_date DESC LIMIT 1`, [name] 
    );
...
app.get("/membership/history/:name", async (req, res) => {
  
  const name = req.params.name
  try {
    const memberShipData = await connectionPool.query(
      `SELECT pt.name, pt.packages_name, pt.created_date,p.price
      FROM payment_test pt
      INNER JOIN packages p on p.packages_name = pt.packages_name 
      WHERE pt.name = $1 AND pt.status = 'success'  
      ORDER BY pt.created_date DESC `, [name] 
    );

    const data = memberShipData.rows; 
    // console.log(memberShipData);
